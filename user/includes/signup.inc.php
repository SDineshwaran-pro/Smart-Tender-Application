    ...
    if (empty($id) || empty($name) || empty($email) || empty($password) || empty($passwordRepeat)) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) { //Check if the statement has been prepared or not
            ...
            if ($resultCheck > 0) {
                ...
                $sql = "INSERT INTO organization (OrgID, OrgName, OrgEmail, OrgPass, Per) VALUES (?, ?, ?, ?, ?)";
                $stmt =  mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    mysqli_stmt_bind_param($stmt, "sssss", $id, $name, $email, $hashedPwd, $percent); //Instead of inserting the password directly insert the hashed password
                    mysqli_execute($stmt);
